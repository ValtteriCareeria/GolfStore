...
// UUSI IMPORT: Tuotepalvelu tarvitaan setSold-kutsuihin
...
      addressService.setToken(token);
      // Asetetaan token myös ProductService:lle
...
    addressService.setToken(token);
    // Varmistetaan, että token on asetettu ProductService:lle
    productService.setToken(token); 
...
      
      // -------------------------------------------------------------
      // UUSI LOGIIKKA: Piilotetaan tuotteet listauksesta tilauksen jälkeen
      // -------------------------------------------------------------
      
      const productIdsToHide = cart.map(item => item.productId);
      
      // Kutsutaan setSold(productId) jokaiselle tuotteelle ostoskorissa.
      // HUOM: Oletetaan, että setSold() vähentää InventoryCountin nollaan
      // tai asettaa muun piilotuslipun (esim. IsSold=true/IsActive=false).
      await Promise.all(productIdsToHide.map(async (productId) => {
        try {
            // TÄRKEÄ OLETUS: setSold hoitaa InventoryCountin päivityksen.
            await productService.setSold(productId); 
        } catch (error) {
            // Huomautus: Jos setSold epäonnistuu jossain tuotteessa (esim. jo piilotettu), 
            // tilaus on silti jo luotu, joten jatketaan prosessia.
            console.error(`Virhe tuotteen ${productId} piilotuksessa:`, error);
        }
      }));
...
      setMessage?.("Tilaus onnistui! Tuotteet on nyt poistettu myyntilistalta.");
      setIsPositive?.(true);
      setShowMessage?.(true);
